...
describe("nOrg", () => {
  ...
  describe("node properties:", () => {
    ...
    it("sorts properties by key", () => {
      ...
      expect(node.$properties()).to.deep.equal([
        "Bah-Property",
        "Bar-Property"
      ]);
    });
    ...
    it("Throws an error for empty property names", () => {
      expect(() => node.$newProperty("")).to.throw(
        "Must provide a property name"
      );
    });
  });
  ...
  describe("node children:", () => {
    ...
    it("removes itself from its parent", () => {
      ...
      expect(node.$childTail.$prevSibling.$basename).to.equal(
        node.$childHead.$basename
      );
      expect(node.$childHead.$nextSibling.$basename).to.equal(
        node.$childTail.$basename
      );
  ...
  describe("nodes from objects:", () => {
    ...
    it("object children are converted to nodes", () => {
      ...
      expect(child.$childHead.$nextSibling["Bar-Property"]).to.equal(
        "Bar Property"
      );
  ...
  describe("moving nodes:", () => {
    it("demotes a node with previous siblings", () => {
      ...
      expect(parent.$parent.$childTail.$basename).to.equal(
        parent.$nextSibling.$basename
      );
    ...
    it("demotes node into a node with children", () => {
      ...
      expect(
        parent.$childHead.$nextSibling.$nextSibling.$nextSibling.$basename
      ).to.equal(node.$basename);
    ...
    it("promotes a node with to the middle", () => {
      ...
      expect(prevSibling.$childHead.$nextSibling.$basename).to.equal(
        prevSibling.$childTail.$basename
      );
      expect(prevSibling.$childTail.$prevSibling.$basename).to.equal(
        prevSibling.$childHead.$basename
      );
    ...
    it("moves up a middle node to first node", () => {
      ...
      expect(top.$childTail.$prevSibling.$basename).to.equal(
        top.$childHead.$nextSibling.$basename
      );
  ...
  describe("cursor:", () => {
    ...
    it("can move down to next parent from last child", () => {
      ...
      top.cursorTo(node);
      top.cursorDown();
      expect(node.$cursorObject.$basename).to.equal(
        node.$parent.$nextSibling.$basename
      );
      expect(node.isCursor()).to.be.false;
  ...
  describe("properties cursor:", () => {
    ...
    it("can move right into expanded properties", () => {
      ...
      expect(
        node.$cursorObject[node.$cursorObject.$properties()[node.$cursorIndex]]
      ).to.equal("Bah Property");
    ...
    it("can move up into expanded property", () => {
      ...
      expect(node.$cursorObject.$properties()).to.deep.equal(
        node.$properties()
      );
    });
