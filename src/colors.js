 ...
 */
export function filterAvailableColors(
  theme = THEME,
...
export function orderColorVariants(
  ...
  fontSize = "0.875rem",
  bold,
  variantsOrder = VARIANTS.light,
  colors = COLORS
) {
  const backgroundParsed = new axe.commons.color.Color(
    ...parseHexColor(background)
  );

  /* Construct a mapping of {color: [variants...]}
     ordered by both color order and variant order */
  const colorVariantsOrder = Object.fromEntries(
    colorOrder
      .map(color => [
        color,
        variantsOrder
